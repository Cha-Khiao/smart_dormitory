...
export default function MyRoomPage() {
  ...
  const handlePaymentSubmit = async (e: React.FormEvent) => {
    ...
    const toastId = toast.loading("กำลังส่งข้อมูลการชำระเงิน...");
    ...
    try {
      ...
      const res = await fetch("/api/bills", {
        ...
        body: JSON.stringify({ id: selectedBill._id, status: "paid", paymentMethod, slipUrl })
      });

      if (res.ok) {
        // 2. ส่งข้อความแจ้งเตือนแอดมินในแชท
        await fetch("/api/messages", {
          method: "POST", headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            bookingId: selectedBill.bookingId, senderId: (session?.user as any).id, senderName: "System", senderRole: "customer",
            text: `💸 แจ้งชำระเงินค่าห้องเดือน ${selectedBill.month} เรียบร้อยแล้ว (ช่องทาง: ${paymentMethod}) ยอดชำระ: ${selectedBill.totalAmount.toLocaleString()} บาท \nดูสลิป: ${slipUrl}`
          })
        });
